...
import type { PigmentMsdsDocument } from "@/types/admin";
import {
  deletePigmentMsdsAction,
  getPigmentMsdsDownloadUrlAction,
  uploadPigmentMsdsAction,
} from "@/app/admin/actions";
import { getErrorMessage } from "@/lib/errors";

interface MsdsDocumentsPanelProps {
  pigmentId: string;
  documents: PigmentMsdsDocument[];
}
...
export function MsdsDocumentsPanel({ pigmentId, documents }: MsdsDocumentsPanelProps) {
  ...
  async function handleUpload(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await uploadPigmentMsdsAction(pigmentId, formData);
      if (result.ok) {
  ...
  async function handleOpen(documentId: string) {
    ...
    try {
      const result = await getPigmentMsdsDownloadUrlAction(documentId);
      if (result.ok) {
  ...
  async function handleDelete(documentId: string) {
    ...
    try {
      const result = await deletePigmentMsdsAction(pigmentId, documentId);
      if (result.ok) {
        router.refresh();
      } else {
        setError(result.error);
            ...
            >
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-sm text-red-700 border border-red-200 rounded-lg hover:bg-red-50 disabled:opacity-50"
                >
